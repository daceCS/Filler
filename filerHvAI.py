...
class Cell:
    def __init__(self):
        self.captured = False
        self.value = 0
        self.color = pygame.Color(set_color())
...
grid_size = 4
board = [[Cell() for i in range(grid_size)] for i in range(grid_size)]
...
def move(player, color, i, j):
    ...
    if i + 1 < 4 and board[i + 1][j].color == pygame.Color(color) and not board[i + 1][j].captured:
        ...
        board[i + 1][j].color = pygame.Color(color)
    ...
    if j + 1 < 4 and board[i][j + 1].color == pygame.Color(color) and not board[i][j + 1].captured:
        ...
        board[i][j + 1].color = pygame.Color(color)
...
def actions(gameState):
    ...
    for col in range(4):
        for row in range(4):
            ...
            if gameState[col][row].value == 1 or gameState[col][row].value == -1:
                if gameState[col][row].color == pygame.Color('red'):
                    red_c += 1
                if gameState[col][row].color == pygame.Color('green'):
                    green_c += 1
                if gameState[col][row].color == pygame.Color('purple'):
                    purple_c += 1
                if gameState[col][row].color == pygame.Color('blue'):
                    blue_c += 1

    if red_c == 0:
        child_arr.append('red')
    if green_c == 0:
        child_arr.append('green')
    if blue_c == 0:
        child_arr.append('blue')
    if purple_c == 0:
        child_arr.append('purple')


    return child_arr


def result(gameState, move, player, i, j):
    newState = copy.deepcopy(gameState)
    if player == 1:
        for x in range(4):
            for y in range(4):
                if newState[x][y].value == 1:
                    newState[x][y].color = pygame.Color(move)
    elif player == -1:
        for x in range(4):
            for y in range(4):
                if newState[x][y].value == -1:
                    newState[x][y].color = pygame.Color(move)
    if i + 1 < 4 and newState[i + 1][j].color == pygame.Color(move) and not newState[i + 1][j].captured:
        newState[i + 1][j].value = player
        newState[i + 1][j].color = pygame.Color(move)
        newState[i + 1][j].captured = True

        result(newState, move, player, i + 1, j)
    if i - 1 >= 0 and newState[i - 1][j].color == pygame.Color(move) and not newState[i - 1][j].captured:
        newState[i - 1][j].value = player
        newState[i - 1][j].color = pygame.Color(move)
        newState[i - 1][j].captured = True

        result(newState, move, player, i - 1, j)
    if j + 1 < 4 and newState[i][j + 1].color == pygame.Color(move) and not newState[i][j + 1].captured:
        newState[i][j + 1].value = player
        newState[i][j + 1].color = pygame.Color(move)
        newState[i][j + 1].captured = True

        result(newState, move, player, i, j + 1)
    if j - 1 >= 0 and newState[i][j - 1].color == pygame.Color(move) and not newState[i][j - 1].captured:
        newState[i][j - 1].value = player
        newState[i][j - 1].color = pygame.Color(move)
        newState[i][j - 1].captured = True
...
def minimax(gameState, depth, maximizingPlayer):
    newState = copy.deepcopy(gameState)
    if depth == 0 or terminal(newState):
        return [value(newState), None]
    if maximizingPlayer == 1:
        maxEval = -infinity
        move = ''
        for a in actions(newState):

            NewGameState = result(newState, a, maximizingPlayer, 3, 0)
            eval = minimax(NewGameState, depth - 1, -1)[0]

            if eval > maxEval:
                maxEval = eval
                move = a
        arr = [maxEval, move]
        print(actions(newState))
        print(maxEval, maximizingPlayer, move)
        return arr
        ...
        for a in actions(newState):
            NewGameState = result(newState, a, maximizingPlayer, 0, 3)
            eval = minimax(NewGameState, depth - 1, 1)[0]
            if eval < minEval:
                minEval = eval
                move = a
...
def aiturn(gameState):
    ...
    print('best ' + str(minimax(newBoard, 2, 1)[1]))
